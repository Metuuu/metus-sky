      ...
      Pass {    
        ...
        uniform float3 _PlanetPos;
         ...
         vertexOutput vert(vertexInput input) 
         {
            ...
            float3 normalDirection = normalize(float3(mul(float4(input.normal, 0.0), modelMatrixInverse.vertex)));
            //float3 normalDirection = normalize(input.normal) // <- tää oikeesti vaan kommenteissa ei tuo ylempi
